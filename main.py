...
def _env_int(key: str) -> int:
    raw = _env_require(key)
    try:
        return int(raw)
    except ValueError:
        raise RuntimeError(f"❌ .env faylida '{key}' raqam bo'lishi kerak: '{raw}'")

API_ID         = _env_int("API_ID")
API_HASH       = _env_require("API_HASH")
BOT_TOKEN      = _env_require("BOT_TOKEN")

# ── SUPER_ADMIN_ID va ADMIN_IDS .env dan o'qiladi ────────
SUPER_ADMIN_ID = _env_int("SUPER_ADMIN_ID")
