...
async def init_db():
    async with aiosqlite.connect(DB_NAME, timeout=30) as db:
        ...
        try:
            ...
            logger.warning("scan_resume statusini 'error' ga o'zgartirish muvaffaqiyatsiz: %s", e)
        await db.execute("""
        ...
        try:
            ...
            logger.debug("WAL checkpoint muvaffaqiyatsiz (normal): %s", e)
        await db.commit()
...
async def check_and_record_alert_hit(alert_id: int, msg_id: int, source: str, sender_id: int) -> bool:
    async with aiosqlite.connect(DB_NAME, timeout=30) as db:
        ...
        try:
            ...
            logger.warning("alert_hits yozishda xato (alert_id=%s, msg_id=%s): %s", alert_id, msg_id, e)
            return False
