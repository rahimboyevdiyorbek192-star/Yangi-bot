...
async def _check_batch_alerts(batch: list):
    ...
    try:
        ...
        logger.warning("_check_batch_alerts xato: %s", e)
...
def _excel_safe(val) -> str:
    """Excel/CSV formula injection oldini oladi.
    = @ + - bilan boshlanadigan matnlarni apostrofga o'rab qaytaradi."""
...
_pc_link_cache: dict = {}
_pc_link_lock = asyncio.Lock()  # Race condition oldini olish uchun
                ...
                async for msg in userbot.iter_messages(
                    ...
                    if msg.sender_id not in seen_ids:
                        ...
                        if sender and not getattr(sender, 'bot', False):
                            ...
                            _un = sender.username   or ""
                            # O'chirilgan hisob: ism ham, username ham yo'q → API yo'q
                            if getattr(sender, 'deleted', False) or (not _fn and not _ln and not _un):
                            ...
                            if _fn or _ln or _un or _bio:
                                ...
                                ws.append([
                                    count,
                                    _excel_safe(_fn), _excel_safe(_ln),
                                    _excel_safe("@" + _un if _un else ""),
                                    sender.id,
                                    ("+" + _ph) if _ph else "",
                                    "❌",
                                ...
                                await db_mod.save_user_to_bank(
                                    sender.id, str(target_group), _fn, _ln,
                                    "@" + _un if _un else "",
                                    _ph, _bdate, _bio,
...
async def search_keywords_local(keyword_str: str, days: int = None):
    ...
    if days:
        ...
        date_clause = "AND mc.msg_date >= ?"
        date_param  = [cutoff]

    def _fts_escape(kw: str) -> str:
        return '"' + kw.replace('"', '""') + '"'

    fts_match = " OR ".join(_fts_escape(kw) for kw in keywords)
    fts_query = f"""
        SELECT mc.sender_id, mc.sender_name, mc.sender_username,
               mc.text, mc.msg_date, mc.source
        FROM messages_fts fts
        JOIN messages_cache mc ON mc.id = fts.rowid
    ...
    async with aiosqlite.connect(db_mod.DB_NAME, timeout=30) as db:
        try:
            async with db.execute(fts_query, [fts_match] + date_param) as cur:
                rows = await cur.fetchall()
        except Exception:
            # FTS5 indeksi yo'q yoki xato → LIKE bilan fallback
            ...
            fallback = f"""
                ...
                FROM messages_cache
                WHERE ({like_clauses})
                {date_clause.replace('mc.', '')}
                ORDER BY msg_date DESC LIMIT 1000
            """
            async with db.execute(fallback, like_params + date_param) as cur:
                rows = await cur.fetchall()
